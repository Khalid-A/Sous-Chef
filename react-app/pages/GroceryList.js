        ...
        ingrMappings.doc(ingredient).get().then((snapshot) =>{
            ...
            if (this.state.editBeforeText != "") {
                ...
                if (this.state.editBeforeText != this.state.newIngredient) {
                    ...
                    removeGroceryListItem(this.state.editBeforeText, this.props.userID);
                    this.setState({
                        editBeforeText: ""
                    });
                }
            ...
            if (standardQuantity) {
                ...
                if (containsNumber) {
                    ...
                    rawTokens.shift();
                    ingredientName = rawTokens.join(" ");
                }
                else {
                    ingredientName = this.state.newIngredient;
                }
                addGroceryListItem(
                    ingredientName,
                    number, // 1 if no number is found
                    this.props.userID
                );
                this.setState({
                    errorMessage: "(Ingredient or units not recognized.)"
                })
            }
        });
    }
